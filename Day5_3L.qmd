...
path <- file.path("/home", "david", "Desktop", "BISCC", "Aurora3L")
...
write.csv(UnstainedSignature, "data/Unstained3L.csv", row.names=FALSE)                                     
```
...
TheUnstained <- read.csv("data/Unstained3L.csv", check.names=FALSE)
...
Panel <- read.csv(Panel, check.names=FALSE) |> dplyr::filter(Day %in% c(1,2,3,4,5))
...
Hotspot <- MagesCauldron(panelfluors=TheFluorophores, unstained = Unstained, returnType = "data")

#write.csv(Hotspot, "/home/david/Desktop/BISCC3L_Day5.csv", row.names=FALSE)
